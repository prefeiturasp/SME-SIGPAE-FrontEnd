...
export const cepCalculator = (setDesabilitaEndereco) =>
  createDecorator({
...
export const buscaCEP = async (cep, values, setDesabilitaEndereco) => {
  if (cep?.length === 9) {
...
export const carregaListaCompletaInformacoesNutricionais = async (
  ...
  const responseInformacoes = await getInformacoesNutricionaisOrdenadas();
...
export const validaProximo = (values, errors, stepAtual) => {
  const validaStepMap = [
...
export const validaProximoIdentificacaoProduto = (values, errors) => {
...
export const validaProximoInformacoesNutricionais = (values, errors) => {
  return (
    Object.keys(errors).length !== 0 ||
    !values.porcao ||
...
export const validaAssinarEnviar = (values, errors, arquivo) => {
  return (
    Object.keys(errors).length !== 0 ||
...
export const geraInitialValuesCadastrar = (ficha) => {
...
export const geraInitialValuesCorrigir = (ficha) => {
  const initialValues = {
...
export const geraInitialValuesDetalharEAnalisar = (ficha) => {
  const initialValues = {
    ...geraInitialValuesCadastrar(ficha),
    marca: ficha.marca?.nome,
    fabricante: ficha.fabricante?.nome,
...
export const carregaTagsCollapses = (ficha, setConferidos) => {
  const stateConferidos = {
...
export const carregarArquivo = async (urlArquivo) => {
  const arquivo = Array({
...
export const ehInformacaoNutricional = (key) => {
  return (
  ...
  Object.keys(values).map((key) => {
    if (initialValues[key] !== values[key]) {
      if (key === "alergenicos" || key === "gluten") {
        payload[key] = stringToBoolean(values[key]);
...
const gerarCamposObrigatoriosRascunho = (values, produtosOptions) => {
  return {
...
const gerarCamposDetalhesProduto = (values) => {
  return {
    prazo_validade: values.prazo_validade || "",
    numero_registro: values.numero_registro || "",
    agroecologico: stringToBoolean(values.agroecologico),
    organico: stringToBoolean(values.organico),
    mecanismo_controle: values.mecanismo_controle || undefined,
    componentes_produto: values.componentes_produto || "",
    alergenicos: stringToBoolean(values.alergenicos),
    ingredientes_alergenicos: values.ingredientes_alergenicos || "",
    gluten: stringToBoolean(values.gluten),
    lactose: stringToBoolean(values.lactose),
    lactose_detalhe: values.lactose_detalhe || "",
  };
};
const gerarCamposInformacoesNutricionais = (values) => {
  return {
...
const gerarCamposConservacao = (values, ehPereciveis) => {
  return {
...
const gerarCamposTemperaturaTransporte = (values, ehPereciveis) => {
  return {
...
const gerarCamposArmazenamento = (values) => {
  return {
    embalagem_primaria: values.embalagem_primaria || "",
    embalagem_secundaria: values.embalagem_secundaria || "",
  };
};
const gerarCamposEmbalagemRotulagem = (values, ehPereciveis) => {
  return {
    ...
    produto_eh_liquido: stringToBoolean(values.produto_eh_liquido),
...
const gerarCamposResponsavelTecnico = (values, arquivo) => {
  return {
...
const gerarCamposModoPreparo = (values) => {
  return {
    modo_de_preparo: values.modo_de_preparo || "",
  };
};
const gerarCamposOutrasInformacoes = (values) => {
  return {
    informacoes_adicionais: values.informacoes_adicionais || "",
  };
};
const retornaValorSeCategoriaPereciveis = (values, ehPereciveis, campo) =>
  ehPereciveis ? values[campo] : undefined;
export const formataInformacoesNutricionais = (values) => {
...
export const inserirArquivoFichaAssinadaRT = (files, setArquivo) => {
  setArquivo(files);
};
export const removerArquivoFichaAssinadaRT = (setArquivo) => {
  setArquivo([]);
};
...
export const imprimirFicha = (uuid, numero, setCarregando) => {
