...
describe("Teste Dashboard Gestão Produto - Erro editais", () => {
  beforeEach(async () => {
    ...
    getProdutosAguardandoAnaliseReclamacao.mockResolvedValue({
      data: {
        detail: "Erro ao carregar produtos em aguardando análise reclamação",
      },
      status: 400,
    });
  ...
  it("renderiza erro `Erro ao carregar produtos aguardando análise reclamação. Tente novamente mais tarde.`", async () => {
    await awaitServices();
    expect(
      screen.getByText(
