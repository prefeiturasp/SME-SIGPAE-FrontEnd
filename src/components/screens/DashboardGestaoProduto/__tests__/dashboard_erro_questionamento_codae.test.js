...
describe("Teste Dashboard Gestão Produto - Erro editais", () => {
  beforeEach(async () => {
    ...
    getProdutosQuestionamentoDaCODAE.mockResolvedValue({
      data: { detail: "Erro ao carregar produtos em questionamento da CODAE" },
      status: 400,
    });
  ...
  it("renderiza erro `Erro ao carregar produtos em questionamento da CODAE. Tente novamente mais tarde.`", async () => {
    await awaitServices();
    expect(
      screen.getByText(
