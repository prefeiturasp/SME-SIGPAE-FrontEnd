...
describe("Teste Dashboard Gestão Produto - Erro editais", () => {
  beforeEach(async () => {
    ...
    getProdutosPendenteHomologacao.mockResolvedValue({
      data: { detail: "Erro ao carregar produtos pendente homologação" },
      status: 400,
    });
  ...
  it("renderiza erro `Erro ao carregar produtos pendente homologação. Tente novamente mais tarde.`", async () => {
    await awaitServices();
    expect(
      screen.getByText(
