...
describe("Testes de comportamento para componente - ModalRelatorio Consolidado do Recreio nas Férias", () => {
  const mockOnClose = jest.fn();
  const mockOnSubmit = jest.fn();
  ...
  it("não permite selecionar um grupo 6 e não habilita o botão de gerar relatório", () => {
    const radio = screen.getByLabelText("Grupo 6 (CIEJA, CMCT)");
    expect(radio).toBeDisabled();
...
describe("Testes de comportamento para componente - ModalRelatorio Unificado do Recreio nas Férias", () => {
  const mockOnClose = jest.fn();
  const mockOnSubmit = jest.fn();
  ...
  it("permite selecionar um grupo 4 habilita o botão de gerar relatório", () => {
    const radio = screen.getByLabelText(
      "Grupo 4 (CEU EMEF, CEU GESTAO, EMEF, EMEFM)",
    );
    fireEvent.click(radio);
  ...
  it("nao exibe os campos de período no relatório unificado", () => {
    expect(
      screen.queryByText("Selecione o período de lançamento:"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByPlaceholderText("Data inicial"),
    ).not.toBeInTheDocument();
    expect(screen.queryByPlaceholderText("Data final")).not.toBeInTheDocument();
  });
});
