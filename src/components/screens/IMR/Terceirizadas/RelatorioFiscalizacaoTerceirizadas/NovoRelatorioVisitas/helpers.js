...
const formatOcorrenciasSim = (values) => {
...
const formatOcorrencias = (values) => {
  let values_ = deepCopy(values);
  let respostas = [];
  let ocorrenciasNao = [];
  let grupos = {};
  const getGrupoRespostas = (tipoOcorrenciaUUID) => {
  ...
  const processRespostas = (grupo, tipoOcorrenciaUUID, indexGrupo) => {
...
export const validarFormulariosTiposOcorrencia = (values, tiposOcorrencia) => {
