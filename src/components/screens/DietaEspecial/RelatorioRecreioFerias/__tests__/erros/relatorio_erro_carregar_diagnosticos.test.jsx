...
describe("Verifica comportamento da interface ao receber retorno de erro do endpoint de alergias e intolerâncias", () => {
  beforeEach(async () => {
    mock.onGet("/lotes-simples/").reply(200, mockLotesSimples);
    mock.onGet("/classificacoes-dieta/").reply(200, mockGetClassificacaoDieta);
    mock.onGet("/alergias-intolerancias/").reply(400, []);
    mock
  ...
  it("Verifica renderização de mensagem de erro", async () => {
    await waitFor(() => {
      expect(
        screen.getByText("Erro ao carregar diagnósticos.")
      ).toBeInTheDocument();
    });
  });
});
