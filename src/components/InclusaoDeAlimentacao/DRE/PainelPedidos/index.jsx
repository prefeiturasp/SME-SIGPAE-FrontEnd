...
import { dataAtualDDMMYYYY, formatarOpcoesLote } from "src/helpers/utilities";
...
export const PainelPedidos = ({ ...props }) => {
  ...
  const [filtros, setFiltros] = useState(
    props.filtrosProps || { lote: undefined },
  );
  ...
  const filtrarPrazo = async (chave, { filtros, busca, page }) => {
    ...
    const data = await dreListarSolicitacoesDeInclusaoDeAlimentacao(
  ...
  useEffect(() => {
    getLotesAsync();
    const paramsFromPrevPage = props.filtrosProps || { lote: undefined };
    CARDS.forEach(({ chave }) =>
      filtrarPrazo(chave, {
