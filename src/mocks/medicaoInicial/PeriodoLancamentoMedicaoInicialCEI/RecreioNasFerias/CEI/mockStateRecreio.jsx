export const mockLocationStateGrupoRecreioNasFerias = {
  periodo: "Recreio nas Férias",
  mesAnoSelecionado:
    "Mon Dec 08 2025 00:00:00 GMT-0400 (Horário Padrão do Amazonas)",
  status_periodo: "MEDICAO_EM_ABERTO_PARA_PREENCHIMENTO_UE",
  status_solicitacao: "MEDICAO_EM_ABERTO_PARA_PREENCHIMENTO_UE",
  escola: "CEI DIRET ODILIA ALVES ALMEIDA SANTOS",
  justificativa_periodo: null,
  ehEmeiDaCemei: false,
  grupo: "Recreio nas Férias - de 0 a 3 anos e 11 meses",
  solicitacaoMedicaoInicial: {
    escola: "CEI DIRET ODILIA ALVES ALMEIDA SANTOS",
    escola_uuid: "61c4812e-82ae-4387-aa33-e1f0f42d6163",
    tipos_contagem_alimentacao: [],
    responsaveis: [
      {
        nome: "Teste",
        rf: "1231231",
      },
    ],
    ocorrencia: null,
    logs: [
      {
        status_evento_explicacao: "Em aberto para preenchimento pela UE",
        usuario: {
          uuid: "78201bfb-ea1c-4fad-b7f6-d7d5d5353ef2",
          cpf: null,
          nome: "SUPER USUARIO ESCOLA CEI",
          email: "escolacei@admin.com",
          date_joined: "10/07/2020 13:15:18",
          registro_funcional: "0000110",
          tipo_usuario: "escola",
          cargo: "Diretor",
          crn_numero: null,
          nome_fantasia: null,
        },
        criado_em: "22/01/2026 12:07:35",
        descricao:
          "Solicitação #63B20 -- Escola CEI DIRET ODILIA ALVES ALMEIDA SANTOS -- 12/2025",
        justificativa: "",
        resposta_sim_nao: false,
      },
    ],
    alunos_periodo_parcial: [],
    historico: null,
    escola_eh_emebs: false,
    escola_cei_com_inclusao_parcial_autorizada: false,
    escola_possui_alunos_regulares: true,
    sem_lancamentos: false,
    justificativa_sem_lancamentos: null,
    justificativa_codae_correcao_sem_lancamentos: null,
    recreio_nas_ferias: {
      uuid: "bcaf57b0-8073-4440-9b22-cbe37f8b6e1c",
      id: 57,
      titulo: "Recreio nas Férias - JAN 2026",
      data_inicio: "08/12/2025",
      data_fim: "28/12/2025",
      unidades_participantes: [
        {
          id: 456,
          uuid: "7b843aa2-c3b1-47a0-b80d-673659972ac6",
          num_inscritos: 100,
          num_colaboradores: 10,
          liberar_medicao: true,
          cei_ou_emei: "N/A",
          tipos_alimentacao: {
            colaboradores: [
              {
                uuid: "83fefd96-e476-42a0-81fc-75b9853b726c",
                nome: "Lanche 4h",
              },
              {
                uuid: "5d1304c8-77a8-4c96-badb-dd2e8c1b76d5",
                nome: "Lanche",
              },
            ],
            inscritos: [
              {
                uuid: "5d1304c8-77a8-4c96-badb-dd2e8c1b76d5",
                nome: "Lanche",
              },
              {
                uuid: "65f11f11-630b-4629-bb17-07c875c548f1",
                nome: "Refeição",
              },
              {
                uuid: "5aa2c32b-1df2-46b6-b2e7-514b885fa9a4",
                nome: "Sobremesa",
              },
              {
                uuid: "8e0917e0-70d0-41a8-a5bb-f85bbdd790bb",
                nome: "Colação",
              },
              {
                uuid: "96cad31e-acb8-4dc3-8861-df0731b375ea",
                nome: "Desjejum",
              },
              {
                uuid: "5bd9ad5c-e0ab-4812-b2b6-336fc89886b1",
                nome: "Refeição da tarde",
              },
              {
                uuid: "7990ca99-55b7-4fc7-b5cc-8a51b23d0102",
                nome: "Almoço",
              },
            ],
          },
          unidade_educacional: {
            uuid: "61c4812e-82ae-4387-aa33-e1f0f42d6163",
            nome: "CEI DIRET ODILIA ALVES ALMEIDA SANTOS",
            codigo_eol: "400189",
          },
          lote: {
            uuid: "1f06b334-cbd1-40c5-85c4-6a3d1926805b",
            nome: "LOTE 10 A",
            nome_exibicao: "LOTE 10 A - MP",
          },
        },
      ],
      criado_em: "20/01/2026 10:56:03",
      alterado_em: "22/01/2026 12:07:14",
    },
    criado_em: "22/01/2026 12:07:35",
    uuid: "63b205e2-fe2c-489b-8279-47d71213939f",
    ano: "2025",
    mes: "12",
    status: "MEDICAO_EM_ABERTO_PARA_PREENCHIMENTO_UE",
    com_ocorrencias: false,
    ue_possui_alunos_periodo_parcial: false,
    logs_salvos: false,
    dre_ciencia_correcao_data: null,
    rastro_lote: 17,
    rastro_terceirizada: 5,
    dre_ciencia_correcao_usuario: null,
    relatorio_financeiro: null,
  },
  recreioNasFerias: true,
};

export const mockLocationStateGrupoColaboradores = {
  periodo: "Colaboradores",
  mesAnoSelecionado:
    "Wed Oct 01 2025 00:00:00 GMT-0300 (Horário Padrão de Brasília)",
  status_periodo: "MEDICAO_EM_ABERTO_PARA_PREENCHIMENTO_UE",
  status_solicitacao: "MEDICAO_EM_ABERTO_PARA_PREENCHIMENTO_UE",
  escola: "CEI DIRET JOAQUIM GOUVEIA FRANCO JR., VER.",
  justificativa_periodo: null,
  ehEmeiDaCemei: false,
  tiposAlimentacao: [
    {
      uuid: "83fefd96-e476-42a0-81fc-75b9853b726c",
      nome: "Lanche 4h",
    },
    {
      uuid: "5d1304c8-77a8-4c96-badb-dd2e8c1b76d5",
      nome: "Lanche",
    },
  ],
  grupo: "Colaboradores",
  solicitacaoMedicaoInicial: {
    escola: "CEI DIRET JOAQUIM GOUVEIA FRANCO JR., VER.",
    escola_uuid: "939243aa-95d4-432f-8f21-1fafd8f3284d",
    tipos_contagem_alimentacao: [],
    responsaveis: [
      {
        nome: "Responsavel AAAAS",
        rf: "3443242",
      },
    ],
    ocorrencia: null,
    logs: [
      {
        status_evento_explicacao: "Em aberto para preenchimento pela UE",
        usuario: {
          uuid: "78201bfb-ea1c-4fad-b7f6-d7d5d5353ef2",
          cpf: null,
          nome: "ESCOLA CEI ADMIN",
          email: "escolacei@admin.com",
          date_joined: "10/07/2020 13:15:18",
          registro_funcional: "0000110",
          tipo_usuario: "escola",
          cargo: "ADMINISTRADOR",
          crn_numero: null,
          nome_fantasia: null,
        },
        criado_em: "11/02/2026 11:33:30",
        descricao:
          "Solicitação #D1031 -- Escola CEI DIRET JOAQUIM GOUVEIA FRANCO JR., VER. -- 10/2025",
        justificativa: "",
        resposta_sim_nao: false,
      },
    ],
    alunos_periodo_parcial: [],
    historico: null,
    escola_eh_emebs: false,
    escola_cei_com_inclusao_parcial_autorizada: false,
    escola_possui_alunos_regulares: true,
    sem_lancamentos: false,
    justificativa_sem_lancamentos: null,
    justificativa_codae_correcao_sem_lancamentos: null,
    recreio_nas_ferias: {
      uuid: "88244981-e695-4dcc-8919-8bf7e3a146c5",
      id: 7,
      titulo: "Recreio nas Férias - OUT 2025",
      data_inicio: "01/10/2025",
      data_fim: "15/10/2025",
      unidades_participantes: [
        {
          id: 11,
          uuid: "ebf065c2-b05e-4a78-9311-8d6dbb0cd825",
          num_inscritos: 100,
          num_colaboradores: 15,
          liberar_medicao: true,
          cei_ou_emei: "N/A",
          tipos_alimentacao: {
            colaboradores: [
              {
                uuid: "83fefd96-e476-42a0-81fc-75b9853b726c",
                nome: "Lanche 4h",
              },
              {
                uuid: "5d1304c8-77a8-4c96-badb-dd2e8c1b76d5",
                nome: "Lanche",
              },
            ],
            inscritos: [
              {
                uuid: "5d1304c8-77a8-4c96-badb-dd2e8c1b76d5",
                nome: "Lanche",
              },
              {
                uuid: "65f11f11-630b-4629-bb17-07c875c548f1",
                nome: "Refeição",
              },
              {
                uuid: "5aa2c32b-1df2-46b6-b2e7-514b885fa9a4",
                nome: "Sobremesa",
              },
              {
                uuid: "8e0917e0-70d0-41a8-a5bb-f85bbdd790bb",
                nome: "Colação",
              },
              {
                uuid: "5bd9ad5c-e0ab-4812-b2b6-336fc89886b1",
                nome: "Refeição da tarde",
              },
              {
                uuid: "96cad31e-acb8-4dc3-8861-df0731b375ea",
                nome: "Desjejum",
              },
              {
                uuid: "7990ca99-55b7-4fc7-b5cc-8a51b23d0102",
                nome: "Almoço",
              },
            ],
          },
          unidade_educacional: {
            uuid: "939243aa-95d4-432f-8f21-1fafd8f3284d",
            nome: "CEI DIRET JOAQUIM GOUVEIA FRANCO JR., VER.",
            codigo_eol: "400153",
          },
          lote: {
            uuid: "036ccfe3-9ae8-4f22-a0bb-47a3e0e998f4",
            nome: "3567-2",
            nome_exibicao: "3567-2 - IP",
          },
        },
      ],
      criado_em: "11/02/2026 11:30:10",
      alterado_em: "11/02/2026 11:30:10",
    },
    criado_em: "11/02/2026 11:33:30",
    uuid: "d1031e15-e191-4bf6-9350-cf9ab688d64d",
    ano: "2025",
    mes: "10",
    status: "MEDICAO_EM_ABERTO_PARA_PREENCHIMENTO_UE",
    com_ocorrencias: false,
    ue_possui_alunos_periodo_parcial: false,
    logs_salvos: false,
    dre_ciencia_correcao_data: null,
    rastro_lote: 20,
    rastro_terceirizada: 5,
    dre_ciencia_correcao_usuario: null,
    relatorio_financeiro: null,
  },
  recreioNasFerias: true,
};
